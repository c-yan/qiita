...
`go` `get` は以前と同じように動作しますし、バージョンを明示的した `go` `mod` `download` と`go` `list` `-m` も同様です.
...
[`TempDir`](https://tip.golang.org/pkg/io/ioutil/#TempDir) は、予測可能なプレフィックスとサフィックスを名前に持つディレクトリを作成できるようになりました. TempFile と同様に、パターンに '\*' が含まれている場合は、最後の '\*' がランダムな文字列に置き換えられます.
