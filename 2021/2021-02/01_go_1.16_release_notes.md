...
モジュールモードでは、`go` コマンドは、非 ASCII 文字や先頭がドット文字(`.`)であるパス要素を含むインポートパスを禁止するようになりました. これらの文字を含むモジュールパスは既に禁止されています([モジュールパスとバージョン](https://tip.golang.org/ref/mod#go-mod-file-ident)を参照してください). ですので、この変更はモジュールのサブディレクトリ内のパスにのみ影響します.
